...
For the Java backend, the compiler generates a JVM entry point for a top-level
...
For example, this program prints a greeting when given a name and otherwise
...
A function merely named `main` is still a normal callable function if it does
not match the entry-point contract. For example, `fun main(): Program`, a
private `main`, a `main` with multiple parameters, or one returning
`Effect[int]` does not produce a JVM launcher.
