...
private fun expressionLongOrShortToString(expression: ExpressionWithReturnType,
        ...
        if (isOneLinerExpression(expression, assertions)) {
            expressionPrefix + buildIndent(indent) + expressionToString(expression, assertions, unions, methodsToRewrite, packageName, longLambdas)
        } else {
            when (expression) {
                is IfExpressionWithReturnType -> {
                    val condition = expressionToString(expression.condition, assertions, unions, methodsToRewrite, packageName, longLambdas)
                    buildIndent(indent) + "if " + condition + ":\n" +
                            expressionLongOrShortToString(expression.trueBranch.expression, expressionPrefix, assertions, unions, methodsToRewrite, packageName, longLambdas, indent + 1, depth + 1) +
                            buildIndent(indent) + " else:\n" +
                            expressionLongOrShortToString(expression.falseBranch.expression, expressionPrefix, assertions, unions, methodsToRewrite, packageName, longLambdas, indent + 1, depth + 1)
                }
                ...
                is AssertExpressionWithReturnType -> {
                    ...
                    val message = if (messageExpression != null) {
                        expressionLongOrShortToString(messageExpression, "", assertions, unions, methodsToRewrite, packageName, longLambdas, indent + 1, depth + 1)
                    } else {
                        "\"<no message>\""
                    }
                    buildIndent(indent) + "if not " + expressionLongOrShortToString(checkExpression, "", assertions, unions, methodsToRewrite, packageName, longLambdas, indent + 1, depth + 1) + ": " +
                            "raise AssertionError($message)\n" +
                            expressionLongOrShortToString(returnExpression, expressionPrefix, assertions, unions, methodsToRewrite, packageName, longLambdas, indent, depth)
                }
...
private fun expressionToString(expression: ExpressionWithReturnType,
        ...
        when (expression) {
            ...
            is IfExpressionWithReturnType -> "(${expressionToString(expression.trueBranch.expression, assertions, unions, methodsToRewrite, packageName, longLambdas)}) if (${expressionToString(expression.condition, assertions, unions, methodsToRewrite, packageName, longLambdas)}) else (${expressionToString(expression.falseBranch.expression, assertions, unions, methodsToRewrite, packageName, longLambdas)})"
