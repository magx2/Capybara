...
Starting async work is explicit and returns an inert handle:

```capybara
fun Effect[T].start(): Async[T] = <native>
```

Observing async results is explicit and effectful:

```capybara
fun all(tasks: List[Async[T]]): Effect[List[Result[T]]] = <native>
fun any(tasks: List[Async[T]]): Effect[Result[T]] = <native>
fun Async[T].join(): Effect[Result[T]] = <native>
```

`Async[T]` is an inert handle in pure code. `start` returns the handle directly
instead of wrapping it in another `Effect`; holding, passing, or composing an
`Async[T]` value must not observe its result or poll its completion state.
`join`, `all`, and `any` are the boundaries that may block, await, or otherwise
cooperate with the backend scheduler.
...
`all` joins every async handle and returns a list of `Result` values in the
same order as the input list. It returns an empty list for an empty input.
`any` joins the first async handle completed by the backend runtime and returns
that handle's `Result`. When `any` receives an empty list, it returns `Error`
with kind `capy.lang.async.empty`.
...
- If `any([])` is evaluated, it returns `Error` with kind
  `capy.lang.async.empty`.
...
structured concurrency in v1. There is also no `Async.is_done` polling API in
v1; completion is observed through `join`.
